# Runnable Examples

Minimal examples demonstrating loop patterns without API keys.

| Example | Pattern | Command |
|---------|---------|---------|
| [reflection-loop](reflection-loop/) | Reflection | `python run.py` |
| [research-loop](research-loop/) | Research | `python run.py` |
| [verification-loop](verification-loop/) | Verification | `python run.py` |
| [loopnet-explore](loopnet-explore/) | Dataset explore + histograms | `python explore.py --plot-dir ../../docs/loopnet/histograms` |

Optional plots: `pip install matplotlib datasets`
| [compose-loop](compose-loop/) | Nested/sequential composition | `python run.py` |
| [loopforge-scaffold](loopforge-scaffold/) | Scaffold LSS YAML from patterns | `python run_demo.py` |

Requires: see each example README. Full path: [contributions/REPRODUCE.md](../contributions/REPRODUCE.md).

→ [Implementations](../implementations/README.md)
