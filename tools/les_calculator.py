...
# LES-1.0 category weights (sum = 1.0)
...
def compute_les(spec: dict[str, Any]) -> tuple[float, dict[str, float]]:
    """
    Compute composite LES using LES-1.0 formula:
    LES = sum(w_i * N_i) where N_i in [0, 1]
    Returns (composite_0_to_100, category_scores).
    """
...
def format_report(
    ...
    lines = [
        f"Loop Engineering Score (LES-1.0)",
        f"{'=' * 40}",
        f"Loop:   {loop_name}",
        f"Source: {source}",
        f"LES:    {les:.1f} / 100",
