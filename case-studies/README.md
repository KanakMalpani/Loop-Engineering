# Case Studies

Deep analyses of real-world feedback systems mapped to Loop Engineering framework.

New submissions: use [TEMPLATE.md](TEMPLATE.md) for the L = (S, A, O, T, E, M, τ) mapping.

---

| Study | Domain | Loop Type | Composite LES |
|-------|--------|-----------|---------------|
| [OpenAI Deep Research](openai-deep-research.md) | AI research | Research + reflection | 76 |
| [AlphaGo Self-Play](alphago-self-play.md) | Game AI | Evolutionary | 72 |
| [Toyota Production System](toyota-production-system.md) | Manufacturing | Reflective + HITL | 81 |
| [GitHub Pull Requests](github-pull-requests.md) | Software | Multi-agent + verify | 73 |
| [SpaceX Iteration](spacex-iteration.md) | Hardware | Rapid test loops | 79 |
| [Scientific Peer Review](scientific-peer-review.md) | Science | Debate + HITL | 68 |
| [Startup PMF Loops](startup-pmf-loops.md) | Business | Exploration + measure | 74 |
| [Autonomous Coding Agents](autonomous-coding-agents.md) | Software | Reflective + verify | 76 |

Each study includes: architecture diagram, feedback analysis, optimization strategy, memory model, success factors, failure modes, and LES breakdown.

→ [D-D-M-I-S Framework](../framework/README.md)
