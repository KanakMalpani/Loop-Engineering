#!/usr/bin/env python3
"""LE-OP-04 v0.1: compare evaluator merge policies (false-pass rate)."""
...
OUT = ROOT / "benchmarks" / "evaluator-composition" / "results-v0.1.json"

# Synthetic cases: (syntax, tests, rubric) — rubric overlaps with tests on "correctness"
CASES = [
...
def double_count_avg(scores: dict[str, float]) -> tuple[float, bool]:
    # Invalid: counts tests + rubric overlap on correctness twice
...
def partition_merge(scores: dict[str, float]) -> tuple[float, bool]:
    # Valid: syntax gate AND merge of disjoint behavioral + synthesis dims
    if scores["syntax"] < 0.75:
        return scores["syntax"], False
    behavioral = scores["tests"]
    synthesis = scores["rubric"] * 0.5 + scores["tests"] * 0.5  # overlap damped in merge only
...
def run_demo() -> dict:
    ...
    return {
        "benchmark_id": "evaluator-composition-v0.1",
        "threshold": THRESHOLD,
        "policies": results,
        "summary": {
...
def main() -> int:
    ...
    return 0 if report["summary"]["partition_merge_beats_double_count"] else 1
